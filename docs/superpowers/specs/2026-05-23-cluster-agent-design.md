...
| Claude (via Agent SDK) | LLM | `CLAUDE_CODE_OAUTH_TOKEN` (active, `sk-ant-oat01-*` from `claude setup-token`). `ANTHROPIC_API_KEY` is documented Doppler fallback. See § 3.6. |
...
### 3.6 Authentication — Max subscription via Agent SDK OAuth (active path)

The Agent SDK reads `CLAUDE_CODE_OAUTH_TOKEN` natively. The token is a
1-year `sk-ant-oat01-*` credential generated by `claude setup-token` on the
operator's laptop — purpose-built for headless/CI/container use.

Operator setup: `claude setup-token` → copy the printed `sk-ant-oat01-*`
value → paste into Doppler `cluster-agent/prd.CLAUDE_CODE_OAUTH_TOKEN` →
deploy. Until the operator pastes the real value, the placeholder
`__PLACEHOLDER_OPERATOR_FILL__` keeps P0 running (no LLM calls in P0 anyway).

Billing: calls bill against the operator's Max subscription. Until
**June 15, 2026** they share the interactive Claude Code rate-limit pool;
after June 15 they move to a separate $100/mo Agent SDK bucket.

**Critical SDK behavior — silent-shadow footgun:** if `ANTHROPIC_API_KEY` is
also set in the container env, the SDK silently uses it instead of
`CLAUDE_CODE_OAUTH_TOKEN`, burning API-key credit without any warning.
`docker-compose.yaml` passes only one at a time. `main.py` adds a fail-fast
`RuntimeError` on startup if both happen to be set simultaneously (catches
future operator or compose regressions).

`ANTHROPIC_API_KEY` remains set in Doppler as a documented fallback. To
switch: swap the one compose env line
(`CLAUDE_CODE_OAUTH_TOKEN → ANTHROPIC_API_KEY`) and re-deploy.

#### 3.6.1 Historical context — "deferred to June 15" conclusion (RESOLVED 2026-05-25)

Earlier (also 2026-05-25, during Task 22.5) we concluded OAuth was
not possible in-container until June 15. That conclusion was wrong: we
were looking at the Agent SDK Python package parameters (which expose no
auth args) and at `~/.claude/.credentials.json` (which macOS Claude Code
does not create). We missed the `CLAUDE_CODE_OAUTH_TOKEN` env var path.

`claude setup-token` generates a token that sidesteps both issues: it's a
standalone opaque `sk-ant-oat01-*` string the Agent SDK reads from env — no
credentials file, no SDK parameter plumbing. June 15 only changes billing.

Practical rollout schedule (updated):

| Period | LLM cadence |
|---|---|
| **May 23 – May 31 (P0)** | No LLM calls — pure foundation work. OAuth token placeholder. |
| **June 1 onwards (P1)** | OAuth active. Manual + hourly Mode A on dev. Until June 15 shares interactive pool. |
| **June 15 onwards** | Separate $100/mo Agent SDK billing bucket. Full cadence. |
...
| Key | Used by |
|---|---|
| `CLAUDE_CODE_OAUTH_TOKEN` | LLM calls — **active auth path** (`sk-ant-oat01-*` from `claude setup-token`; see § 3.6) |
| `ANTHROPIC_API_KEY` | LLM calls — **documented fallback only** (API key stays in Doppler; NOT passed to container env by default — see § 3.6 for operator-side compose toggle) |
