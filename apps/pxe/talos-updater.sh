...
#
# Runs on the NAS via a TrueNAS cronjob (registered by `phase apps`).
#
...
#   5. Render the standalone Talos menu (talos-menu.ipxe), served over
#      HTTP by the pxe container's nginx — chained by our top-level
#      menu.ipxe's Talos entry when the operator wants the version
#      picker / diagnostic view.
#
...
ASSETS_DIR="${ASSETS_DIR:-/mnt/tank/system/pxe/http/talos}"
...
download_if_missing() {
    ...
    # Also maintain "latest" symlinks so menus/talos.ipxe can reference
    # stable URLs (http://.../talos/vmlinuz, http://.../talos/initramfs)
    # without knowing the current version.
    ln -sfn "${ver}/vmlinuz-${ARCH}" "$ASSETS_DIR/vmlinuz"
    ln -sfn "${ver}/initramfs-${ARCH}.xz" "$ASSETS_DIR/initramfs"
...
# Render the rich standalone Talos menu served from HTTP :8080/talos-menu.ipxe.
# Chained by our top-level menu.ipxe's Talos → :chain-menu item.
render_menus() {
    sid="$1"; default_ver="$2"
    ...
    talos_menu="/mnt/tank/system/pxe/http/talos-menu.ipxe"
    {
        cat <<HEADER
#!ipxe
...
# render_menus() in apps/pxe/talos-updater.sh and re-apply.
...
item --gap
item --gap ─── Navigation ─────────────────────────────────
item return   <-- Return to Homelab main menu
